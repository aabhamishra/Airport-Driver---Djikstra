...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		...
		// throw NoSuchElementException when no vertex containing start or end can be
		// found
		if (vStart == null || vEnd == null) {
			throw new NoSuchElementException();
		}

		// inner class with vertex information
		class vertexInfo {
			Vertex vertex;
			Path path;
		...
		// Priority Queue initialization with lambda expression for editing comparator
		...
		// create hastable from this list
			...
			for (Edge e : current.vertex.edgesLeaving) {
				...
				if (infoTable.get(n).isVisited != true) {
					...
					if (next.cumulativeWeight == Integer.MAX_VALUE
						...
						next.path = new Path(current.path);
						// extend path of next
						next.path.extend(e);
